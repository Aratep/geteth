...
export default (req, res) => {
    if (req._parsedUrl.pathname !== '/' && /.*\/$/.test(req._parsedUrl.pathname)) {
        req._parsedUrl.pathname = req._parsedUrl.pathname.replace(/\/+$/, '');
        res.writeHead(302, {
            Location: req._parsedUrl.format(),
        });
        res.end();
        return;
    }
    ...
    */
    const injectHTML = (data, { title, meta, body, scripts, state }) => {
            ...
            */
            frontloadServerRender(() =>
                ...
                if (context.url) {
                    ...
                    if (context.error_404) res.status(404);
                    res.send(html);
                }
            });
        }
